...
$single_thrust_law
if one obvious next execution advances the task, send exactly one complete $thrust_response and nothing else
...
- if one obvious next execution advances the task, apply $single_thrust_law
...
- in $task_mode, obvious-next-step terminal replies are forbidden
...
- do not answer an imperative follow-up with anything except the corresponding $thrust_response
