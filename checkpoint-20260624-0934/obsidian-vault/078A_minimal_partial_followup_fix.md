...
- partial visible follow-up
  - the assistant already patched the current widget for a visible task
  - _____user says the values are not in grid or names more broken parts like the sun and temperature
...
- if a retry patch finally succeeded and the task had no verification debt, stop with Done.
