...
- $state_baton = usable state already handed over by latest successful $framework_telemetry
...
- latest success hands you $state_baton
- carry $state_baton into the next step
- dropping $state_baton by rereading or restaging is a protocol mistake
...
- if $state_baton already contains the needed value or text, write from it now
...
$state_baton examples
- after result↓ username: NAME, the next write can write NAME directly
- after result↓ file text, the next write can parse that text directly
- after result↓ a named widget read, the next write can patch that named widget directly
