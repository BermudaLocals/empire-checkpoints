...
- hot source after prose slip
  - readWidget or fileRead already succeeded on the same target in this open task
  - the assistant then made a prose-only promise instead of executing
  - _____user says do it, then do it, continue, or execute
  - assistant still mutates that same target now
...
- if a prose-only turn happened after a fresh same-target read, the next do it/continue/execute still uses that same fresh source
...
- rereading the same target after a fresh same-target read plus do it
