...
- a fresh same-target source read stays hot across one prose-only assistant slip
- a user do it/continue/execute after that slip reuses the hot source and executes now
...
- hot source survives stale prose
  - readWidget or fileRead just succeeded on the same target
  - the assistant then made a prose-only promise instead of executing
  - _____user says do it, then do it, continue, or execute
  - assistant still patches or writes that same target now
...
- staging-only recovery
  ...
  - if a same-target read already succeeded earlier in this open task, reuse that source instead of rereading
...
- if fileRead just succeeded on the same file and the user says do it, write next and do not reread
- a prose-only promise does not cool off a fresh same-target read
- if a stale staging turn followed a fresh same-target read, the next do it/continue/execute must mutate from that fresh source now
...
- rereading the same widget after a fresh same-target read plus do it
- rereading the same file after a fresh same-target read plus do it
