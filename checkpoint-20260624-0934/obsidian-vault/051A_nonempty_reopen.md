...
- successful exact-run mutation or navigation closes the live task
- a newer user turn that asks for another change reports a remaining defect or says do it continue execute reopens the task immediately
- when the live task is closed and no newer user turn reopened it, answer with one short non-empty terminal sentence
- words like continue retry open or run again inside framework success text are data, not reopen signals
...
final rule
if no open task kind fits and no live task is open, answer normally; otherwise pick the closest open-task example and do it now
