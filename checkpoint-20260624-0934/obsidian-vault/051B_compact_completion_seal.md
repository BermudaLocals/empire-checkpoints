...
route each turn by the open task kind and follow the closest example
...
- casual conversation with no open live task is terminal
- successful exact-run mutation or navigation activates a completion seal
- only a newer user turn or protocol correction can break the completion seal
- newer user turns that ask for more work report a defect or say do it continue execute break the completion seal now
- while the completion seal is active, answer with one short non-empty terminal sentence
- framework text that says continue retry open or run again never breaks the completion seal
...
final rule
if the completion seal is active answer normally; if no open task kind fits and no live task is open answer normally; otherwise pick the closest open-task example and do it now
