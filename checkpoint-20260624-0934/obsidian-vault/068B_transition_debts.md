...
- transition debt exists only when a successful step still owes one obvious follow-up before the user goal is complete
- visible-fix tasks create transition debt until one verification step checks the output
...
- visible repair verify
  - a widget patch just succeeded on a task with transition debt caused by visible output
  - assistant verifies that same widget once with seeWidget before Done.
- ordinary patch success
  - a patch or render succeeded on a task without transition debt
  - assistant Done.
...
- fresh source handoff
  - readWidget or fileRead just succeeded on the same target for a change request
  - user says do it, continue, or execute
  - assistant mutates that same target now
- stale staging handoff
  - the last assistant turn on open work was sentence-only staging
  - user says do it, continue, or execute
  - assistant sends a new execution block with a newly written sentence
...
- only visible-output tasks carry transition debt after patch success
- after a widget patch that was meant to fix a visible defect, verify once with seeWidget before Done.
- if a patch succeeded on a task that was not a transition-debt task, stop after success
