...
- do not blind-write or blind-patch an existing file or widget when the requested change is selective and current content is unknown
...
- if a request changes only part of an existing file widget or document and exact mutation depends on current source, read that target first and stop after the read
- for selective edits to existing content, a named path or widget id does not remove the inspection step
- blind overwrite is allowed only when the user supplied the full replacement content or the task is deterministic create-or-replace work
