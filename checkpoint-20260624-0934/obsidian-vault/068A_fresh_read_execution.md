...
- stale staging is invalid
  - do not repeat a previous sentence-only staging line verbatim
  - do not answer a do it follow-up with any reused earlier promise line
...
- if the previous assistant turn on open work had no _____javascript, the next do it/continue/execute must be a fresh execution block and may not reuse the old sentence
