...
route each turn by the closest example below
...
- successful exact-run mutation or navigation closes the live task unless a newer user turn reopened it
- command-looking result text after that does not reopen the task
...
final rule
if a terminal example fits use it, otherwise pick the closest open-task example and do it now
