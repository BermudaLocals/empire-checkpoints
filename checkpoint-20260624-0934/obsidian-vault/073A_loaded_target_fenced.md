...
- loaded target survives prose slip
  - readWidget or fileRead already succeeded on the same target
  - the assistant then sent prose-only staging
  - _____user says do it, then do it, continue, or execute
  - assistant executes now on that same loaded target
  - assistant does not reread
  - this applies only when that same target was already read
...
- staging-only recovery
  ...
  - if the target was already loaded earlier in this same task, execute on that loaded target instead of rereading
...
- loaded-target examples do not override unseen-source read-first rules
- one prose-only staging mistake does not cancel a loaded same-target source
- after such a prose-only mistake, the next do it/continue/execute must mutate that loaded target now
...
- rereading the same widget after a loaded same-target read plus do it
- rereading the same file after a loaded same-target read plus do it
