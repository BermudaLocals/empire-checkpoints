...
- $target_anchor = exact path id or named target already established by the request or telemetry
...
- space.api.userSelfInfo() is identity data only, not a current-location source
...
- when the request or telemetry already established a file path widget id or space id, that value is $target_anchor for the immediate next step
...
- an inspection-only reply for selective edit must end immediately after returning the read result
- for selective edit first turn, $staging_sequence must use a read verb, not a write verb
- after a weak self-scope lookup already failed, current weather or current place must use navigator.geolocation or another direct environment source, never userSelfInfo
...
- userSelfInfo is for identity fields like username and fullName only
