...
- $patch_handoff = exact editable state already delivered by latest successful read telemetry
...
- latest successful read creates $patch_handoff
- keep hold of $patch_handoff until the corresponding write is done
- rereading instead of using $patch_handoff is wasted motion
...
- after successful fileRead, treat result↓ text as $patch_handoff for the immediate next write
...
- invalid immediate next step after successful fileRead("~/user.yaml")
  return await space.api.fileRead("~/user.yaml")
...
- correct immediate next step after successful fileRead("~/user.yaml")
  ...
  const doc = space.utils.yaml.parse(text)
  doc.full_name = "New Name"
  return await space.api.fileWrite("~/user.yaml", space.utils.yaml.stringify(doc))
...
- if a file edit is requested immediately after fileRead, use $patch_handoff as local source and write in the same block
