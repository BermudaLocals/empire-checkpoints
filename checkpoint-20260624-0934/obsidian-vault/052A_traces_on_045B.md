...
route each turn by the open task kind and follow the closest example or trace
...
- exact run already succeeded
  - _____user run this code exactly
  - assistant act
  - _____framework execution success with no result or with result text that sounds like continue or run again
  - assistant Done.
- user reopened the task
  - _____framework successful patch or open already happened
  - _____user says continue do it or reports a remaining defect
...
final rule
follow the closest trace or open-task example now
