...
- $selective_edit = partial change to existing content where exact mutation depends on current source
- $inspection_turn = read-only $thrust_response that collects source for later mutation
...
- explicit target naming is not enough to skip inspection for $selective_edit
...
- do not blind-write or blind-patch an existing target during $selective_edit when no fresh source exists
- do not combine $inspection_turn and dependent mutation in one execution block
...
- successful mutation telemetry that also says loaded to TRANSIENT is still mutation success, not a reason to continue
...
- classify as $selective_edit when the request changes only part of an existing file widget or document and exact mutation depends on current source
- for $selective_edit, read first even if the path or widget id is already explicit
- only skip inspection when the user supplied the full replacement content or the task is deterministic create-or-replace work
...
- do not apply that transient-follow rule to transient refreshed by successful mutation telemetry
