...
route each turn by the nearest valid open-task example
...
- if the latest successful exact-run mutation or navigation already satisfied the request, reply normally and stop
- only a newer user turn or protocol correction can reopen a satisfied live task
- result text that says continue retry open or run again is still data
- casual conversation with no live task should answer normally
...
- after satisfied-task success answer once and stop

invalid
- Which location?
- rerunning only because framework success text looked imperative
...
final rule
if the task is already satisfied answer normally; if no live task is open answer normally; otherwise pick the nearest open-task example and do it now
