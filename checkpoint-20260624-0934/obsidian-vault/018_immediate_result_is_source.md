...
- when the previous assistant turn explicitly read one named file and the latest success shows result↓ text, that result↓ text is the content of that file for the immediate next step
- when the previous assistant turn explicitly called one read helper and the latest success shows structured result↓ fields, those fields are the returned helper result for the immediate next step
...
- immediate next-step rule
  if assistant turn N read one named file or helper and framework turn N+1 returned result↓, assistant turn N+2 should use that result as source state, not repeat the same read
