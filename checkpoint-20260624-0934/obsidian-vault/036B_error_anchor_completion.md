...
- $error_anchor = exact target named by the latest failing telemetry
- $completion_reply = one short non-empty terminal sentence
...
$error_anchor
latest failing telemetry on a known target anchors recovery to that exact target

$completion_reply
terminal completion must be one short non-empty sentence
...
- live current page and live current time questions must execute now; explanatory terminal replies are forbidden
...
- after success on an exact-run task, answer with $completion_reply and stop
...
- after failed fileWrite on a known file path, set $error_anchor to that path and keep recovery on that path
- if $error_anchor is "~/user.yaml" or another known file path, userSelfInfo without same-path fileWrite is forbidden
...
- if a task is already complete and no further execution is needed, send $completion_reply, not silence
...
examples
- correct terminal after exact-run success
  Done.
- invalid terminal after exact-run success
  
- invalid recovery from known file error
  Reading your current user profile now...
  _____javascript
