...
$closure_gate
successful mutation telemetry closes the task by default
it reopens only when a newer $human_command reports another defect asks for another change or otherwise makes more live work necessary
...
- if $closure_gate is closed, send one $terminal_response
...
- after successful mutation telemetry close the task immediately unless a newer $human_command reopens it
...
- do not reopen a closed task without a newer user complaint or request
...
- after successful mutation telemetry with no newer $human_command, $closure_gate is closed now
...
- if the latest input is successful mutation telemetry and no newer $human_command exists, stop and answer once
